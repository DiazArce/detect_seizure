...
def main ():
    global nb_question,message_numero_question,video,cage  
    ...
    Label(Fen1,text = "Information will be saved in the file:  "+namefile,
    ...
    Ent_enonce = Listbox(top,height=3,width=40) 
    Ent_enonce.insert(END," video "+ __builtin__.video  ) 
    Ent_enonce.insert(END," cage "+  str(__builtin__.cage)  ) 
    Ent_enonce.pack(padx=10,pady=10)
    
    contenu=Frame(Fen1,borderwidth=2,width=80,height=100, relief=GROOVE, bg='ivory')
    contenu.pack(side="top")
    ...
    def enregistrer(): 
        ...
        if nb_question == 2:
            ...
            source.writerow([date,rep1,rep2,rep3,justy,video,cage])
